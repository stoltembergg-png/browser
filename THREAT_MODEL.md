...
| TM-006 | origin A → permission | confused deputy/grant reuse | camera/mic/notification/storage abuse | PR-042: default deny, exact origin/top-level/opener/profile/tab/permission key, user-gesture grant, expiry, one-shot/session/persistent lifetime, revoke and scoped clear | `permission_policy` origin-confusion/default-deny/gesture/expiry/one-shot/revoke/clear tests; hardware integration remains | control implemented for PR-042; engine/hardware adapter pending |
